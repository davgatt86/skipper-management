...
Return ONLY a JSON object — no markdown, no commentary — with exactly these keys:
{
  ...
  "category":    "one of: Statutory, LSA, FFA, Radio, Pollution, Medical, Machinery, Insurance, Equipment, Other",
...
- Statutory: registry, tonnage, measurement, builder's, ILO 188, fishing vessel certificate, compliance documents.
- LSA (life-saving appliances): liferafts, lifejackets, immersion suits, EPIRB, flares, lifebuoys.
- FFA (fire-fighting appliances): portable extinguishers, fixed suppression systems, fire detection.
- Radio: GMDSS, ship radio licence, long range radiotelephone.
- Pollution: MARPOL, anti-fouling / AFS, oil record book, garbage management.
- Medical: ships medical stores, first aid kits.
- Machinery: engine, gearbox, shaft, crane and lifting gear examinations.
- Insurance: insurance and financial-security cover, including wreck removal.
- Equipment: anything else surveyed or serviced that does not fit above.
- Prefer the specific bucket over the general one. A liferaft service is LSA, never Equipment; an extinguisher is FFA, never Equipment.
- If a field is genuinely absent, use null. Do not guess.`
...
export async function parseVesselCertFile(file) {
  ...
  if (!resp.ok) throw new Error(data.error || `Parser error ${resp.status}`)
  const j = extractJson(data.text)
  /* THE SAME ELEVEN THE PAGE OFFERS, less the legacy 'Safety' bucket.
   *
   * These two lists were out of step and it mattered: the prompt offered five
   * categories and the clamp allowed those five, so the reader COULD NOT
   * suggest LSA, FFA, Radio, Pollution, Medical or Machinery — the six that the
   * vessel-certificate work created precisely because "Safety" was a useless
   * bundle of seven. Every liferaft, extinguisher and radio certificate read
   * from a photo landed in Safety or Other and had to be refiled by hand.
   *
   * 'Safety' is deliberately NOT offered: it is kept on the page so old filings
   * still show, but nothing new should be put there.
   *
   * Kept in step with VESSEL_CERT_CATEGORIES in VesselCerts.jsx — if that list
   * changes, this one does too. */
  const CATS = ['Statutory', 'LSA', 'FFA', 'Radio', 'Pollution', 'Medical',

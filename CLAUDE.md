...
### THE PROVIDER WAS NEVER IN THE TREE — and it had been writing null for weeks

David, Sep 2026: *"new certification is not working as it's not linking the
vessel in vessel details to the pages"*, with
`null value in column "vessel_id" of relation "vessel_details" violates
not-null constraint` on save.

**`VesselProvider` WAS IMPORTED INTO `App.jsx` AND NEVER RENDERED.** One line
at the top of the file, and `<AuthProvider>` wrapping `<Routes>` directly. So
every `useCurrentVessel()` in **fifteen pages** read the module default —
`{ current: null, vessels: [], hasVessels: false, showing: 'none' }` — and the
whole app behaved as though the fleet had no boat:

    ORB, OLB, radio log      "No vessel on file. Add one on Vessel."
    self-certification       "Choose a boat first" -- to a ONE-boat fleet
    risk assessments,        "Vessel: the fleet"
      lifting register
    vessel details           saved vessel_id: null into a NOT NULL column

**Only the last one produced an error.** The other four rendered a perfectly
tidy page with the wrong answer on it, which is why it survived: the build
passes, nothing throws, and a context consumer FALLS BACK TO ITS DEFAULT rather
than crashing. That is the worst shape this class of bug takes, and it is the
fifth of the class here after the undefined identifiers, `busy`, `killWhy` and
the const reassignment. **There is no ESLint in this project**, so an unused
import is not flagged by anything.

`test-vessels.mjs` passed throughout — 51 checks, all green, testing
`resolveCurrent` in isolation. **The logic was right; it was never wired in.**
A unit test on a function says nothing about whether anything calls it.

#### AND IT HAD BEEN WRITING NULL SINCE THE DAY IT SHIPPED

The half that matters more. `current` was permanently null, so pages that stamp
the boat on insert stamped nothing — and pages that scope by boat scoped
nothing, which is exactly why nobody noticed: reading behaved as *show
everything*.

**Mounting the provider turns the scoping ON.** Every row written in the
meantime would then have vanished off the page that wrote it:

    vessel_departures   864 of 1,577    AUDACIOUS
    sales_landings       10 of 357      AUDACIOUS + BOY JOHN/ROSEBLOOM
    engine_logs           3             crew_lists            2
    garbage_log           2             landings              2
    vessel_certificates   2             stores_lists          1
    vessel_fuel_log       1             quota_trips           1

So `supabase/vessel_id_backfill_after_provider.sql` is **part of the fix, not a
tidy-up after it**. Shipping the provider alone would have taken an engine log,
two crew lists, two vessel certificates and a stores list off his pages, and it
would have looked like data loss rather than a filter.

**NOTHING IS GUESSED, and the two rules are stage 2's own.** A sales note names
its own boat in `sales_landings.vessel`, so those ten are a real join — which
matters because eight of them are a PAIR fleet, where "the only boat" is no
answer at all; that runs FIRST so the pair is settled before the single-vessel
rule runs. Then a fleet with exactly one boat has exactly one possible answer.
Generated by a loop over every table carrying `vessel_id`, so a table added
later is covered without editing a list.

**Two rows are still null and both are right**: HANSTHOLM's 4 `rota_trips`,
because that fleet has no `vessels` row to point at, and TEST FLEET's one
`quota_manual_stocks`, a pair where the row says nothing about which half.

`test-app-tree.mjs` guards it — every `*Provider` imported by `App.jsx` must
appear as `<Provider` in it, and `VesselProvider` must sit INSIDE
`AuthProvider`, because it reads `appUser.fleet_id` to know whose boats to
fetch. A repo-wide unused-import sweep was written first and thrown away: it
flagged 34, almost all of them harmless `React` imports under the automatic JSX
runtime, and a guard that cries wolf is one nobody keeps.

**Known and NOT acted on**: that sweep did find real dead imports —
`PageHeader` in Users, Settings, Password and AddBoat, `Link` in Closeout and
